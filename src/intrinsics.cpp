...
void translator::add_llvm_memcpy_support() {
  ...
  auto sym4 = symbol_util::create_symbol(
      bool_typet(), "llvm.memcpy.p0i8.p0i8.i64::is_volatile");
  ...
  array_copy_code.operands().push_back(typecast_exprt(
      address_of_exprt(new_arr), pointer_type(signedbv_typet(8))));
  ...
  array_replace_code.operands().push_back(typecast_exprt(
      address_of_exprt(new_arr), pointer_type(signedbv_typet(8))));
  ...
  br_inst->set_condition(not_exprt(
      binary_relation_exprt(len, ID_gt, from_integer(0, len.type()))));
  ...
  goto_functions.function_map["llvm.memcpy.p0i8.p0i8.i64"]
      .set_parameter_identifiers(to_code_type(fn->type));
}
...
void translator::add_llvm_memset_support() {
  ...
  auto new_arr_sym = symbol_util::create_symbol(
      array_typet(signedbv_typet(8), len), "llvm.memset.p0i8.i64::new_arr");
...
void translator::add_llvm_nearbyint_support() {
  ...
  auto sym1 =
      symbol_util::create_symbol(double_type(), "llvm.nearbyint.f64::x");
  sym1.is_parameter = true;
  symbol_table.insert(sym1);
  auto e1 = sym1.symbol_expr();

  auto sym2 =
      symbol_util::create_symbol(double_type(), "llvm.nearbyint.f64::rti");
  symbol_table.insert(sym2);
  auto rti = sym2.symbol_expr();
  tgt = temp_gp.add(goto_programt::make_decl(rti));
...
void translator::add_llvm_copysign_support() {
  ...
  auto expr = ternary_exprt(
      ID_if, extractbit_exprt(y, to_bitvector_type(y.type()).get_width() - 1),
...
void translator::add_llvm_maxnum_support() {
  ...
  //					y,
  //					ternary_exprt(ID_if,
  //							binary_relation_exprt(x,
  //ID_gt, y), 							x,
  // y, 							double_type()), 					double_type()),
  // double_type());
...
void translator::add_llvm_round_support() {
  ...
  temp_gp.add(goto_programt::make_assignment(
      xp, ieee_float_op_exprt(
              e1, ID_floatbv_plus, half_double.to_expr(),
              from_integer(ieee_floatt::ROUND_TO_ZERO, signed_int_type()))));
  ...
  temp_gp.add(goto_programt::make_assignment(
      xp, ieee_float_op_exprt(
              e1, ID_floatbv_minus, half_double.to_expr(),
              from_integer(ieee_floatt::ROUND_TO_ZERO, signed_int_type()))));
...
translator::get_intrinsic_id(const string &intrinsic_name) {
