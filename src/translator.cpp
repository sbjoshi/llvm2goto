...
map<const Argument *, string> translator::func_arg_name_map =
    map<const Argument *, string>();
map<DIScope *, string> translator::scope_name_map = map<DIScope *, string>();
string translator::error_state = "";
map<translator::intrinsics, bool> translator::intrinsic_support_added =
    map<intrinsics, bool>();
  ...
  if (isa<ConstantAggregate>(C)) {
    if (isa<ConstantArray>(C)) {
      ...
      if (C.getType()->isArrayTy())
        list_expr = array_exprt(
            operands, to_array_type(symbol_util::get_goto_type(C.getType())));
      else {
        auto type = symbol_util::get_goto_type(C.getType());
        if (type.id() == ID_struct_tag)
          type = symbol_table
                     .lookup(to_struct_tag_type(type).get_identifier().c_str())
                     ->type;
      ...
      if (C.getType()->isArrayTy())
        expr = array_exprt(
            expr_opnds, to_array_type(symbol_util::get_goto_type(C.getType())));
      else {
        auto type = symbol_util::get_goto_type(C.getType());
    ...
    if (isa<ConstantAggregateZero>(C)) {
      ...
      if (C.getType()->isArrayTy())
        ...
        list_expr = struct_exprt(
            operands, to_struct_type(symbol_util::get_goto_type(C.getType())));
      expr = list_expr;
      ...
      if (CF.getType()->isFP128Ty()) {
        ...
        if (CF.getValueAPF().isNaN())
          ...
          ieee_fl.make_zero();
        else if (CF.getValueAPF().isInfinity() && CF.getValueAPF().isNegative())
...
exprt translator::get_expr_fcmp(const FCmpInst &FCI) {
  ...
  case CmpInst::Predicate::FCMP_OEQ: {
    expr = ternary_exprt(
        ID_if, isnan_exprt(expr_op1), false_exprt(),
        ternary_exprt(ID_if, isnan_exprt(expr_op2), false_exprt(),
                      ieee_float_equal_exprt(expr_op1, expr_op2), bool_typet()),
        bool_typet());
    break;
  }
  ...
  case CmpInst::Predicate::FCMP_ORD: {
    expr =
        ternary_exprt(ID_if, isnan_exprt(expr_op1), false_exprt(),
                      ternary_exprt(ID_if, isnan_exprt(expr_op2), false_exprt(),
                                    true_exprt(), bool_typet()),
                      bool_typet());
    break;
  }
  case CmpInst::Predicate::FCMP_UEQ: {
    expr = ternary_exprt(
        ID_if, isnan_exprt(expr_op1), true_exprt(),
...
exprt translator::get_expr_icmp(const ICmpInst &ICI) {
  ...
  case CmpInst::Predicate::ICMP_UGE: { // TODO:Check if its correct to cast to
                                       // unsigned like this
    expr_op1 = typecast_exprt::conditional_cast(
        expr_op1,
        unsignedbv_typet(ll_op1->getType()->isPointerTy()
                             ? 64
                             : ll_op1->getType()->getIntegerBitWidth()));
    expr_op2 = typecast_exprt::conditional_cast(
        expr_op2,
        unsignedbv_typet(ll_op2->getType()->isPointerTy()
                             ? 64
                             : ll_op2->getType()->getIntegerBitWidth()));
    expr = binary_relation_exprt(expr_op1, ID_ge, expr_op2);
    break;
  }
...
exprt translator::get_expr_fadd(const Instruction &FAI) {
  ...
  expr =
      ieee_float_op_exprt(expr_op1, ID_floatbv_plus, expr_op2, rounding_mode);
  return expr;
}
...
exprt translator::get_expr_fsub(const Instruction &FSI) {
  ...
  auto rounding_mode =
      symbol_table.lookup("__CPROVER_rounding_mode")->symbol_expr();
  expr =
      ieee_float_op_exprt(expr_op1, ID_floatbv_minus, expr_op2, rounding_mode);
  return expr;
}
...
exprt translator::get_expr_fmul(const Instruction &FMI) {
  ...
  auto rounding_mode =
      symbol_table.lookup("__CPROVER_rounding_mode")->symbol_expr();
  expr =
      ieee_float_op_exprt(expr_op1, ID_floatbv_mult, expr_op2, rounding_mode);
  return expr;
}
...
exprt translator::get_expr_udiv(const Instruction &UDI) {
  ...
  expr = typecast_exprt::conditional_cast(
      expr, signedbv_typet(ll_op1->getType()->getIntegerBitWidth()));
  return expr;
}
...
exprt translator::get_expr_bitcast(const BitCastInst &BI) {
  ...
  if (expr.type().id() == ID_pointer || expr.type().id() == ID_array)
    ...
    expr =
        typecast_exprt(expr, pointer_type(symbol_util::get_goto_type(ll_op2)));
...
exprt translator::get_expr_fneg(const Instruction &I) {
  ...
  auto flip_sign =
      from_integer(1llu << (bit_width - 1), signedbv_typet(bit_width));
...
exprt translator::get_expr_sext(const SExtInst &SI) {
  ...
  if (SI.getOperand(0)->getType()->getIntegerBitWidth() == 1) {
    expr = ternary_exprt(ID_if, equal_exprt(expr, from_integer(0, expr.type())),
...
exprt translator::get_expr_zext(const ZExtInst &ZI) {
  ...
  zext_and--;
  auto zext_expr =
      from_integer(zext_and, signedbv_typet(ll_op2->getIntegerBitWidth()));
  ...
  for (auto i = 1u, n = GEPI.getNumOperands(); i < n; i++)
    if (expr.type().id() == ID_pointer)
      ...
      if (expr.type().id() == ID_struct_tag)
        struct_t = to_struct_type(
            symbol_table
                .lookup(
                    to_struct_tag_type(expr.type()).get_identifier().c_str())
                ->type);
      else
...
void translator::trans_alloca(const AllocaInst &AI) {
  ...
  if (alloca_dbg_map.find(&AI) != alloca_dbg_map.end()) {
    ...
    if (!DI->getName().str().compare(""))
      goto L1;
    symbol.name = scope_name_map
                      .find(dyn_cast<DILocalScope>(DI->getScope())
    ...
    if (AI.hasName()) {
      ...
      symbol.name =
          AI.getFunction()->getName().str() + "::" + symbol.base_name.c_str();
    } else
    L1:
      symbol.name =
          AI.getFunction()->getName().str() + "::" + symbol.name.c_str();
  }
  ...
  if (AI.isArrayAllocation()) {
    ...
    goto_program.add(goto_programt::make_assignment(
        code_assignt(
            aux_symbol.symbol_expr(),
            typecast_exprt::conditional_cast(
                address_of_exprt(index_exprt(symbol.symbol_expr(),
...
void translator::trans_br(const BranchInst &BI) {
  ...
  if (BI.isConditional()) {
    ...
    br_instr_target_map[&BI] =
        pair<goto_programt::targett, goto_programt::targett>(cond_true,
                                                             cond_false);
    ...
    br_instr_target_map[&BI] =
        pair<goto_programt::targett, goto_programt::targett>(br_ins, br_ins);
...
///		function_ptr: these are translated in cbmc style, by adding if
/// condtions. 		assert: Instead of a function call, assert instruction
/// must be added. 		assume: Instead of a function call, assume
/// instruction must be added.
void translator::trans_call(const CallInst &CI) {
  ...
  if (called_func) {
    if (called_func->isIntrinsic()) { /// LLVM Intrinsic Functions
    ...
    } else if (!called_func->getName().str().compare(
                   "__assert_fail")) { ///__assert_fail is instrinsic assert of
                                       /// llvm which is equivalent to
                                       /// assert(fail && "Message").
      auto assert_inst = goto_program.add(
          goto_programt::make_assertion(false_exprt(), location));
      auto asrt_comment =
          cast<ConstantDataArray>(cast<ConstantExpr>(CI.getOperand(0))
                                      ->getOperand(0)
                                      ->getOperand(0))
              ->getAsCString();
      auto file_name =
          cast<ConstantDataArray>(cast<ConstantExpr>(CI.getOperand(1))
                                      ->getOperand(0)
                                      ->getOperand(0))
              ->getAsCString();
      auto func_name =
          cast<ConstantDataArray>(cast<ConstantExpr>(CI.getOperand(3))
                                      ->getOperand(0)
                                      ->getOperand(0))
              ->getAsCString();
      ...
      goto_program.update();
                   ...
                   "reach_error")) { /// reach_error is the error definition for
                                     /// SV-COMP 20
      goto_program.add(goto_programt::make_assertion(false_exprt(), location));
      goto_program.update();
    } else if (!called_func->getName().str().compare(
                   "abort")) { /// abort in SV-COMP 20 should stop Verification
                               /// at that point
  ...
  L1:
    ...
    if (is_assert_function(called_val->getName().str())) {
      exprt guard_expr;
      if (called_val->getName().str() ==
          "assert_") // assert_ for Fortran is always pass by address, hence the
                     // implicit dereference.
        guard_expr = typecast_exprt(
            dereference_exprt(get_expr(*CI.getOperand(0))), bool_typet());
      else
        guard_expr = typecast_exprt(get_expr(*CI.getOperand(0)), bool_typet());
      auto assert_inst =
          goto_program.add(goto_programt::make_assertion(guard_expr, location));
      assert_inst->source_location_nonconst().set_comment(
          "assertion " + from_expr(namespacet(symbol_table), "", guard_expr));
      goto_program.update();
      ...
      auto guard_expr =
          typecast_exprt(get_expr(*CI.getOperand(0)), bool_typet());
      goto_program.add(goto_programt::make_assumption(guard_expr, location));
      goto_program.update();
    } else if (is_intrinsic_function(called_val->getName().str())) {
      add_intrinsic_support(*called_func); /// General Intrinsic functions, like
                                           /// fpclassify, fesetround, etc
      make_func_call(CI);
    } else {
      auto ll_func_type =
      ...
      if (!ll_func_type->getReturnType()->isVoidTy()) {
        ...
        goto_program.add(goto_programt::make_decl(
            code_declt(ret_symbol.symbol_expr()), location));
        goto_program.update();
      }
      for (auto func_sym : actual_symbols) {
        ...
        func_calls.insert(
            code_function_callt(lhs_expr, call_func_expr, call_arguments));
      }
      ...
      for (auto actual_func_call : func_calls) {
        goto_programt::targett temp =
            goto_program.add_instruction(FUNCTION_CALL);
        func_targets.push_back(temp);
        goto_programt::targett temp2 = goto_program.add_instruction(GOTO);
        goto_end_targets.push_back(temp2);
      }
      if (!ll_func_type->getReturnType()
               ->isVoidTy()) { /// We should add a new variable to capture the
                               /// return value of the func.
        auto asgn_instr = goto_program.add(goto_programt::make_assignment(
      ...
      goto_programt::targett end =
          goto_program.add(goto_programt::make_skip(location));
      if (ll_func_type->getReturnType()->isVoidTy())
      ...
      for (auto actual_func_call : func_calls) {
        ...
        cond_targets[i]->set_condition(equal_exprt(
            func_expr, address_of_exprt(actual_func_call.function())));
...
void translator::make_func_call(const CallInst &CI) {
  ...
  if (called_func
          ...
          : get_intrinsic_return_type(called_val->getName().str()).id() !=
                ID_void) { /// We should add a new variable to capture the
                           /// return value of the func.
    symbolt ret_symbol;
    if (called_func)
    ...
    if (CI.hasName()) {
      ...
      ret_symbol.name =
          CI.getFunction()->getName().str() + "::" + ret_symbol.name.c_str();
    ret_symbol.location = location;
    if (symbol_table.add(ret_symbol)) {
      error_state = "duplicate symbol names encountered!";
    }
...
void translator::trans_call_llvm_intrinsic(const IntrinsicInst &ICI) {
  ...
  case Intrinsic::memcpy: {
    ...
    auto symbol =
        symbol_util::create_symbol(array_typet(signedbv_typet(8), len_expr));
    if (ICI.hasName()) {
      symbol.base_name = ICI.getName().str();
      symbol.name =
          ICI.getFunction()->getName().str() + "::" + symbol.base_name.c_str();
    } else
      symbol.name =
          ICI.getFunction()->getName().str() + "::" + symbol.name.c_str();
    if (symbol_table.add(symbol)) {
      error_state = "duplicate symbol names encountered!";
    }
    ...
    goto_program.add(
        goto_programt::make_decl(code_declt(symbol.symbol_expr()), location));
  ...
  case Intrinsic::stacksave: {
    ...
    if (ICI.hasName()) {
      ...
      ret_symbol.name =
          ICI.getFunction()->getName().str() + "::" + ret_symbol.name.c_str();
    if (symbol_table.add(ret_symbol)) {
      error_state = "duplicate symbol names encountered!";
    }
    goto_program.add(goto_programt::make_assignment(
        ret_symbol.symbol_expr(),
...
void translator::trans_insertvalue(const InsertValueInst &IVI) {
  ...
  goto_program.add(goto_programt::make_assignment(
      code_assignt(tgt_expr, src_expr), get_location(IVI)));
  goto_program.update();
}
...
void translator::trans_ret(const ReturnInst &RI) {
  if (const auto &ll_op1 = RI.getReturnValue()) {
    ...
    goto_program.add(
        goto_programt::make_set_return_value(ret_expr, get_location(RI)));
  }
...
void translator::trans_store(const StoreInst &SI) {
  ...
  goto_program.add(
      goto_programt::make_assignment(tgt_expr, src_expr, get_location(SI)));
  goto_program.update();
}
...
void translator::set_branches() {
  for (auto br_inst_iter = br_instr_target_map.begin(),
            ie = br_instr_target_map.end();
       br_inst_iter != ie; br_inst_iter++) {
    ...
    if (BI->isConditional()) {
      ...
      //			br_inst_iter->second.first->make_goto(false_taget,
      // guard_expr);
...
void translator::set_switches() {
  for (auto sw_inst_iter = switch_instr_target_map.begin(),
            ie = switch_instr_target_map.end();
       sw_inst_iter != ie; sw_inst_iter++) {
    ...
    for (auto i = 1u, n = SI->getNumCases(); i <= n; i++) {
      auto guard_expr =
          equal_exprt(select_expr, get_expr(*SI->getOperand(i * 2)));
...
void translator::set_function_symbol_value(
    goto_functionst::function_mapt &function_map, symbol_tablet &symbol_table) {
...
bool translator::trans_module() {
  ...
  for (auto F = llvm_module->getFunctionList().begin();
    ...
    goto_functions.function_map.find(F->getName().str())
        ->second.body.swap(goto_program);
    goto_functions.function_map.find(F->getName().str())
        ->second.set_parameter_identifiers(to_code_type(fn->type));
    goto_program.clear();
  }
  if (check_state())
    return true;
  remove_skip(goto_functions); /// Remove and unncecessary skip instructions
  ...
  for (auto &F : *llvm_module)
    for (auto &BB : F)
      for (auto &I : BB) {
        ...
        if (isa<StoreInst>(&I) || isa<AllocaInst>(&I) || isa<BranchInst>(&I) ||
            isa<ReturnInst>(&I) || isa<CallInst>(&I) || isa<SwitchInst>(&I) ||
        ...
        for (const auto &U : I.uses()) {
          ...
          if (UI->getParent() == I.getParent()) {
            for (auto it = I.getIterator(); it != UI->getIterator(); it++) {
              if (isa<StoreInst>(it))
